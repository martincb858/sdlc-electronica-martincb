...
def test_alert_humedad(temperatura: float, humedad: float, capsys) -> None:
    ...
    if detector.evaluate(reading) == "ANOMALIA_HUMEDAD":
        ...
        alert_manager.send_alert(reading.device_id, alerta, reading.humedad, reading.timestamp)
...
def test_alert_throtling(capsys) -> None:
    ...
    reading_1 = SensorReading("BODEGA-ZONA-1", temperatura=36.5, humedad=60.0, timestamp=t0)
    alerta_1 = detector.evaluate(reading_1)
    
    if alerta_1 == "ANOMALIA_TEMPERATURA":
        alert_manager.send_alert(reading_1.device_id, alerta_1, reading_1.temperatura, reading_1.timestamp)
    ...
    reading_2 = SensorReading("BODEGA-ZONA-1", temperatura=36.5, humedad=60.0, timestamp=t1)
    alerta_2 = detector.evaluate(reading_2)
    
    if alerta_2 == "ANOMALIA_TEMPERATURA":
        alert_manager.send_alert(reading_2.device_id, alerta_2, reading_2.temperatura, reading_2.timestamp)
